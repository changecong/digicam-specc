/**************************************************
 * Author: Zhicong Chen -- 10/08/2012 17:14:47
 * Email: chen.zhico@husky.neu.edu
 * Filename: ReadBmp_aux.sc
...
behavior ReadBmp(out char ScanBuffer[IMG_HEIGHT_MDU*8][IMG_WIDTH_MDU*8],
					i_sender out_port)
{
	...
	/* functons used to get the header info from image */

	// Read a reverse word in to structure
	int ReadRevWord() {
		int c;
		c = fgetc(ifp);
		c |= fgetc(ifp) << 8;
